...
<template>
  <v-menu :open-on-hover="!isMobile">
    ...
    <v-list>
      <v-list-item :title="$t('preferences.title')" prepend-icon="mdi-tune-vertical-variant"
        :to="{ name: 'preferences' }" />
      <v-list-item :title="$t('preferences.barButtons.pageInfo')" prepend-icon="mdi-information-outline"
        @click="showInfoDialog = true" v-if="showInfoButton && !showPageInfo" />
      <v-tooltip open-on-click :open-on-hover="false" :text="networkTooltip" location="left">
        <template #activator="{ props }">
          <v-list-item v-bind="props" :title="$t('preferences.barButtons.networkStatus')" :prepend-icon="networkIcon"
            v-if="!showNetwork" />
        </template>
      </v-tooltip>
    </v-list>
